...
def graphDrawing(skeleton, edgeList, eps):
    ...
    for i, edge in enumerate(edgeList):
        ...
        c = colorMat[i, :]
        for point in edge:
            graphImg[point[0] - 1:point[0] + 2, point[1] - 1:point[1] + 2, :] = np.stack(
                (c[0] * np.ones((3, 3)), c[1] * np.ones((3, 3)), c[2] * np.ones((3, 3)))).T
    return graphImg
...
def branching(searchBranchPoint, skeletonTemp, edgeList, edgeNumber):
    branchNeighborMatrix = np.array(skeletonTemp[searchBranchPoint[0] - 1:searchBranchPoint[0] + 2,
                                    searchBranchPoint[1] - 1:searchBranchPoint[1] + 2], copy=True)
    ...
    if (branchNearbyBranch.any()):
        ...
        branchConnected4, branchConnected4Num = ndimage.label(branchNeighborMatrixNew,
